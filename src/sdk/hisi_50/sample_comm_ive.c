...
HI_VOID SAMPLE_COMM_IVE_DestroyImage(IVE_IMAGE_S *pstImg)
{
    HI_MPI_SYS_MmzFree(pstImg->u32PhyAddr[0], pstImg->pu8VirAddr[0]);
}
...
HI_VOID SAMPLE_Limit_Region_Position(SAMPLE_RECT_ARRAY_S *pstRect, HI_U16 u16RegionX, HI_U16 u16RegionY,
                                     HI_U16 u16RegionWidth, HI_U16 u16RegionHeight,
                                     HI_U16 u16ThreshWidth, HI_U16 u16ThreshHeight)
{
    HI_S32 s32RegionCount = 0, i, k;
    HI_U16 u16RegionX1 = u16RegionX + u16RegionWidth, u16RegionY1 = u16RegionY + u16RegionHeight;
    HI_S32 s32MaxTwoIdx[MAX_REGION_NUM] = {-1}, s32MaxSqu[MAX_REGION_NUM] = {-1}, s32tmpSqu = 0;
    ...
    for(i = 0; i < pstRect->u16Num; i++)
	{
        pstRect->astRect[i].astPoint[0].s32X = PD_MAX(pstRect->astRect[i].astPoint[0].s32X, u16RegionX);
        pstRect->astRect[i].astPoint[0].s32X = PD_MIN(pstRect->astRect[i].astPoint[0].s32X, u16RegionX1);
        pstRect->astRect[i].astPoint[0].s32Y = PD_MAX(pstRect->astRect[i].astPoint[0].s32Y, u16RegionY);
        pstRect->astRect[i].astPoint[0].s32Y = PD_MIN(pstRect->astRect[i].astPoint[0].s32Y, u16RegionY1);

        pstRect->astRect[i].astPoint[1].s32X = PD_MAX(pstRect->astRect[i].astPoint[1].s32X, u16RegionX);
        pstRect->astRect[i].astPoint[1].s32X = PD_MIN(pstRect->astRect[i].astPoint[1].s32X, u16RegionX1);
        pstRect->astRect[i].astPoint[1].s32Y = PD_MAX(pstRect->astRect[i].astPoint[1].s32Y, u16RegionY);
        pstRect->astRect[i].astPoint[1].s32Y = PD_MIN(pstRect->astRect[i].astPoint[1].s32Y, u16RegionY1);

        pstRect->astRect[i].astPoint[2].s32X = PD_MAX(pstRect->astRect[i].astPoint[2].s32X, u16RegionX);
        pstRect->astRect[i].astPoint[2].s32X = PD_MIN(pstRect->astRect[i].astPoint[2].s32X, u16RegionX1);
        pstRect->astRect[i].astPoint[2].s32Y = PD_MAX(pstRect->astRect[i].astPoint[2].s32Y, u16RegionY);
        pstRect->astRect[i].astPoint[2].s32Y = PD_MIN(pstRect->astRect[i].astPoint[2].s32Y, u16RegionY1);

        pstRect->astRect[i].astPoint[3].s32X = PD_MAX(pstRect->astRect[i].astPoint[3].s32X, u16RegionX);
        pstRect->astRect[i].astPoint[3].s32X = PD_MIN(pstRect->astRect[i].astPoint[3].s32X, u16RegionX1);
        pstRect->astRect[i].astPoint[3].s32Y = PD_MAX(pstRect->astRect[i].astPoint[3].s32Y, u16RegionY);
        pstRect->astRect[i].astPoint[3].s32Y = PD_MIN(pstRect->astRect[i].astPoint[3].s32Y, u16RegionY1);
        if (pstRect->astRect[i].astPoint[2].s32X - pstRect->astRect[i].astPoint[0].s32X > 0 &&
            pstRect->astRect[i].astPoint[2].s32Y - pstRect->astRect[i].astPoint[0].s32Y > 0 &&
            pstRect->astRect[i].astPoint[2].s32X - pstRect->astRect[i].astPoint[0].s32X > u16ThreshWidth &&
            pstRect->astRect[i].astPoint[2].s32Y - pstRect->astRect[i].astPoint[0].s32Y > u16ThreshHeight)
        {
            s32tmpSqu = (pstRect->astRect[i].astPoint[2].s32X-pstRect->astRect[i].astPoint[0].s32X)*(pstRect->astRect[i].astPoint[2].s32Y - pstRect->astRect[i].astPoint[0].s32Y);
            for (k = 0; k < MAX_REGION_NUM; k++)
            {
                if(s32tmpSqu > s32MaxSqu[k])
				{
                    s32MaxTwoIdx[k] = i;
                    s32MaxSqu[k] = s32tmpSqu;
                    break;
                }
/*
                    if(s32tmpSqu > s32MaxSqu[0]){
                        s32MaxTwoIdx[0] = i;
                        s32MaxSqu[0] = s32tmpSqu;
                    }
                    if(s32MaxSqu[0] != s32tmpSqu && s32tmpSqu > s32MaxSqu[1]){
                        s32MaxTwoIdx[1] = i;
                        s32MaxSqu[1] = s32tmpSqu;
                    }
*/
            }
            // printf("limit region position: \n index:%d, squ:%d, \nmax1_idx:%d, max1_squ:%d, \nmax2_idx:%d, max2_squ:%d,\n", i, s32tmpSqu, s32MaxTwoIdx[0], s32MaxSqu[0], s32MaxTwoIdx[1], s32MaxSqu[1]);
        }
    }
    //printf("limit region position: \n s32MaxSqu[0]:%d, s32MaxSqu[1]:%d, s32MaxSqu[2]:%d, s32MaxSqu[3]:%d\n", s32MaxSqu[0], s32MaxSqu[1], s32MaxSqu[2],  s32MaxSqu[3]);
    for(i = 0; i < MAX_REGION_NUM && -1 != s32MaxTwoIdx[i]; i++)
	{
        pstRect->astRect[s32RegionCount].astPoint[0].s32X = pstRect->astRect[s32MaxTwoIdx[i]].astPoint[0].s32X;
        pstRect->astRect[s32RegionCount].astPoint[0].s32Y = pstRect->astRect[s32MaxTwoIdx[i]].astPoint[0].s32Y;
        pstRect->astRect[s32RegionCount].astPoint[1].s32X = pstRect->astRect[s32MaxTwoIdx[i]].astPoint[1].s32X;
        pstRect->astRect[s32RegionCount].astPoint[1].s32Y = pstRect->astRect[s32MaxTwoIdx[i]].astPoint[1].s32Y;
        pstRect->astRect[s32RegionCount].astPoint[2].s32X = pstRect->astRect[s32MaxTwoIdx[i]].astPoint[2].s32X;
        pstRect->astRect[s32RegionCount].astPoint[2].s32Y = pstRect->astRect[s32MaxTwoIdx[i]].astPoint[2].s32Y;
        pstRect->astRect[s32RegionCount].astPoint[3].s32X = pstRect->astRect[s32MaxTwoIdx[i]].astPoint[3].s32X;
        pstRect->astRect[s32RegionCount].astPoint[3].s32Y = pstRect->astRect[s32MaxTwoIdx[i]].astPoint[3].s32Y;
        s32RegionCount++;
    }
    pstRect->u16Num = s32RegionCount;
}
