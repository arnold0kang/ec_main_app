...
HI_S32 SAMPLE_INNER_CODEC_CfgAudio(AUDIO_SAMPLE_RATE_E enSample, HI_S32 voice)
{
    HI_S32 fdAcodec = -1;
    ...
    //mute right
    ACODEC_VOL_CTRL rCtrl;
    rCtrl.vol_ctrl = 0;
    rCtrl.vol_ctrl_mute = 1;
    ioctl(fdAcodec, ACODEC_SET_ADCR_VOL, &rCtrl);
    int i = 1;
    ioctl(fdAcodec, ACODEC_SET_MICR_MUTE, &i);
...
/* config codec */
HI_S32 SAMPLE_COMM_AUDIO_CfgAcodec(AIO_ATTR_S* pstAioAttr, HI_S32 voice)
{
    ...
    /*** INNER AUDIO CODEC ***/
    s32Ret = SAMPLE_INNER_CODEC_CfgAudio(pstAioAttr->enSamplerate, voice);
    if (HI_SUCCESS != s32Ret)
    {
