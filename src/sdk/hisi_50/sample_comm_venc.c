    ...
    for (i = 0; i < pstStream->u32PackCount; i++)
    {
            ...
            pstStreamBuf->u32PhyAddr + pstStreamBuf->u32BufSize)
        {
                ...
                pstStreamBuf->u32PhyAddr + pstStreamBuf->u32BufSize)
            {
                ...
                s32Ret = fwrite((void *) u32SrcPhyAddr, pstStream->pstPack[i].u32Len - pstStream->pstPack[i].u32Offset,
                                1, pFd);
                if (s32Ret < 0)
                {
...
******************************************************************************/
HI_S32 SAMPLE_COMM_VENC_Start(VENC_CHN VencChn, PAYLOAD_TYPE_E enType, VIDEO_NORM_E enNorm, PIC_SIZE_E enSize,
                              SAMPLE_RC_E enRcMode, HI_U32 u32Profile, ROTATE_E enRotate,
                              HI_U32 frameRate, HI_U32 bitRate, int gop)
{
    HI_S32 s32Ret;
    ...
    switch (enType)
    {
        case PT_H264:
        {
            ...
            if (SAMPLE_RC_CBR == enRcMode)
            {
                ...
                stH264Cbr.u32Gop = gop;
                stH264Cbr.u32StatTime = 1; /* stream rate statics time(s) */
                stH264Cbr.u32SrcFrmRate = (VIDEO_ENCODING_MODE_PAL == enNorm) ? 25 : 30; /* input (vi) frame rate */
                stH264Cbr.fr32DstFrmRate = frameRate; /* target frame rate */
                switch (enSize)
                {
                    case PIC_QCIF:
                        stH264Cbr.u32BitRate = bitRate; /* average bit rate */
                        break;
            ...
            else if (SAMPLE_RC_VBR == enRcMode)
            {
                ...
                {
                    case PIC_QCIF:
                        stH264Vbr.u32MaxBitRate = bitRate; /* average bit rate */
                        break;
                    case PIC_QVGA:    /* 320 * 240 */
                    case PIC_CIF:
            ...
            else if (SAMPLE_RC_AVBR == enRcMode)
            {
                ...
                {
                    case PIC_QCIF:
                        stH264AVbr.u32MaxBitRate = bitRate; /* average bit rate */
                        break;
                    case PIC_QVGA:    /* 320 * 240 */
                    case PIC_CIF:
        ...
        case PT_H265:
        {
            ...
            else if (SAMPLE_RC_AVBR == enRcMode)
            {
                ...
                stH265AVbr.u32Gop = (VIDEO_ENCODING_MODE_PAL == enNorm) ? 50 : 60;
                stH265AVbr.u32StatTime = 2;
...
* note      : rate control parameter need adjust, according your case.
******************************************************************************/
HI_S32 SAMPLE_COMM_VENC_StartEx(VENC_CHN VencChn, PAYLOAD_TYPE_E enType, VIDEO_NORM_E enNorm, PIC_SIZE_E enSize,
                                SAMPLE_RC_E enRcMode, HI_U32 u32Profile, VENC_GOP_ATTR_S *pstGopAttr)
{
    HI_S32 s32Ret;
    VENC_CHN_ATTR_S stVencChnAttr;
    VENC_ATTR_H264_S stH264Attr;
                    ...
                    for (s32Cnt = 0; s32Cnt < 3; s32Cnt++)
                    {
                        switch (s32Cnt)
                        {
                            case 0:
                                ...
                                {
#ifndef __HuaweiLite__
                                    s32Ret = SAMPLE_COMM_VENC_SaveStream(enPayLoadType[i], pFile[i + s32Cnt],
                                                                         &stStream);
#else
...
******************************************************************************/
HI_S32 SAMPLE_COMM_VPSS_SetChnCropProc(VPSS_GRP VpssGrp, VPSS_CHN VpssChn, HI_U32 u32SrcWidth, HI_U32 u32SrcHeigth,
                                       HI_U32 u32DstWidth, HI_U32 u32DstHeigth)
{
